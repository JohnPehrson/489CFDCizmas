function [cells_q,cells_f,cells_g] = applyOutletBC(user_Gamma,P_resevoir,cells_q,cells_f,cells_g,cells_Imax,cells_Jmax,user_Mach)
...
%loop through vertical cells at the outlet
    for j = 3:(cells_Jmax-2) 
        [cells_q(i_gc,j,:),cells_f(i_gc,j,:),cells_g(i_gc,j,:)] = singleCellOutletBC(user_Gamma,cells_q(i_g,j,:),cells_f(i_g,j,:),cells_g(i_g,j,:),P_resevoir,user_Mach);
