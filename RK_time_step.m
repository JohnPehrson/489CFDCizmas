function [cell_q_out,cell_f_out,cell_g_out,Res_out] = RK_time_step(user_Gamma,user_Mach,user_alpha,Pstatic,a_rk,q_freeze,D_freeze,A_cell,dt,x_abcd,y_abcd,f_cNESW,g_cNESW,ind1,ind2,cells_Imax,cells_Jmax)
...
R_rk = zeros(4,4);
    for k = 1:4 %loops through the 4 steps in the RK algorithm
        ...
        q_rk(:,k) = q_freeze(:)-a_rk(k)*(dt/A_cell)*(R_rk(:,k)-D_freeze(:));
            ...
            %reapply BC
            [f_cNESW,g_cNESW] = RK_BC(user_Gamma,user_Mach,user_alpha,Pstatic,ind1,ind2,q_rk(:,k),f_cNESW,g_cNESW,x_abcd,y_abcd);
    end
cell_q_out = q_rk(:,4);
